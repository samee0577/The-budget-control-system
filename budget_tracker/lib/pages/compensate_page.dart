...
class _LentTab extends StatefulWidget {
  final CompensateController controller;
  const _LentTab({required this.controller});

  @override
  State<_LentTab> createState() => _LentTabState();
}

class _LentTabState extends State<_LentTab> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _createCardForm(),
          const SizedBox(height: 24),
          if (widget.controller.cards.isNotEmpty) ...[
            const Text(
              'Active',
              style: TextStyle(
                fontSize: 11,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            widget.controller.totalOwed > 0
                ? '₹${widget.controller.totalOwed.toStringAsFixed(0)}'
                : '₹0',
            style: TextStyle(
              fontSize: 32,
              fontWeight: FontWeight.w500,
              color: widget.controller.totalOwed > 0
      ...
      children: [
        Text(label,
            style: const TextStyle(
                fontSize: 11, color: Color(0xFF6B6B8A))),
        ...
        TextField(
          ...
          style:
              const TextStyle(fontSize: 14, color: Color(0xFFE8E8F5)),
          decoration: InputDecoration(
            hintText: hint,
            hintStyle:
                const TextStyle(color: Color(0xFF3A3A5A)),
            filled: true,
  ...
  Future<void> _addCard() async {
    ...
    await widget.controller.addCard(card);
  ...
  Widget _draggableCardsRow() {
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemCount: widget.controller.cards.length,
        itemBuilder: (context, index) {
          final card = widget.controller.cards[index];
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: const Icon(Icons.close,
                    size: 11, color: Color(0xFF6B6B8A)),
              ),
            ),
          ),
        ],
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            card.itemName,
            style: const TextStyle(
                fontSize: 9, color: Color(0xFF6B6B8A)),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
          ...
          Text(
            ...
            style: const TextStyle(
                fontSize: 12, color: Color(0xFFCC5A7A)),
          ),
        ],
      ),
    );
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                      fontSize: 11, color: Color(0xFF3A3A5A)),
                ),
                SizedBox(height: 32),
              ],
            ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                'To collect: ₹${card.remainingAmount.toStringAsFixed(0)}',
                style: const TextStyle(
                    fontSize: 12, color: Color(0xFFCC5A7A)),
              ),
              ...
              TextField(
                ...
                style:
                    const TextStyle(color: Color(0xFFE8E8F5)),
                decoration: InputDecoration(
                  filled: true,
                  fillColor: const Color(0xFF0F0F1A),
                  border: OutlineInputBorder(
                    ...
                    child: GestureDetector(
                      onTap: () async {
                        final amount = double.tryParse(
                            ...
                            0;
                        if (amount <= 0) return;
                        await widget.controller.returnAmount(
                          cardId: card.id,
  ...
  Widget _transactionLog() {
    return FutureBuilder<List<TransactionEntry>>(
      future: TransactionController.loadTransactions(
          section: 'compensate'),
...
class _BorrowedTab extends StatefulWidget {
  final BorrowController controller;
  const _BorrowedTab({required this.controller});

  @override
  State<_BorrowedTab> createState() => _BorrowedTabState();
}

class _BorrowedTabState extends State<_BorrowedTab> {
  final TextEditingController _itemController = TextEditingController();
  final TextEditingController _personController = TextEditingController();
  final TextEditingController _amountController = TextEditingController();
  bool _isDragOver = false;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            widget.controller.totalBorrowed > 0
                ? '₹${widget.controller.totalBorrowed.toStringAsFixed(0)}'
                : '₹0',
            style: TextStyle(
              fontSize: 32,
              fontWeight: FontWeight.w500,
              color: widget.controller.totalBorrowed > 0
                  ? const Color(0xFFCC5A7A)
                  : const Color(0xFF2EB89A),
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _formField('Item', _itemController, 'e.g. Rent'),
          const SizedBox(height: 10),
          _formField('Person', _personController, 'e.g. Dad'),
          const SizedBox(height: 10),
          _formField('Amount', _amountController, 'e.g. 1000',
              isNumber: true),
          const SizedBox(height: 16),
          GestureDetector(
            onTap: _canAdd ? _addCard : null,
  ...
  Widget _draggableCard(BorrowCard card) {
    return Draggable<BorrowCard>(
      data: card,
      feedback: Material(
        color: Colors.transparent,
        child: _borrowCard(card, isDragging: true),
      ),
      childWhenDragging: Opacity(
        opacity: 0.4,
        child: _borrowCard(card),
      ),
      child: Stack(
        children: [
          _borrowCard(card),
  ...
  Widget _borrowCard(BorrowCard card, {bool isDragging = false}) {
    return Container(
      width: 100,
      margin: const EdgeInsets.only(right: 10),
      padding: const EdgeInsets.all(10),
  ...
  void _showRepayPopup(BorrowCard card) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 4),
              Text(
                'To repay: ₹${card.remainingAmount.toStringAsFixed(0)}',
                style: const TextStyle(
                    fontSize: 12, color: Color(0xFFCC5A7A)),
              ),
              const SizedBox(height: 20),
                    ...
                    child: GestureDetector(
                      onTap: () async {
                        final amount = double.tryParse(
                            ...
                            0;
                        if (amount <= 0) return;
                        await widget.controller.repayAmount(
                          cardId: card.id,
                          repaidAmount: amount,
                        );
                        setState(() {});
...
class _CompensatePageState extends State<CompensatePage>
  ...
  final CompensateController _compensateController = CompensateController();
  final BorrowController _borrowController = BorrowController();
  ...
  Future<void> _load() async {
    await _compensateController.loadCards();
    await _borrowController.loadCards();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        bottom: PreferredSize(
          ...
          child: Container(
            ...
            child: TabBar(
              ...
              labelColor: const Color(0xFFE8E8F5),
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          _LentTab(controller: _compensateController),
          _BorrowedTab(controller: _borrowController),
        ],
      ),
    );
  }
